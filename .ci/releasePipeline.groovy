...
void sbom(Context ctx) {
    stage("Generate SBOM") {
        ...
        sbomTools.install()
        sbomTools.generate("sbom.cdx.json")
        pushToDtRepository(source: "sbom.cdx.json", dest: "monaco/${ctx.version}/sbom.cdx.json")
        if (isRelease(ctx)) {
            ctx.githubRelease.addToRelease(source: "sbom.cdx.json", underName: "sbom.cdx.json")
        }
    }
}
