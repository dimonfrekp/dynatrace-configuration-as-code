...
func TestDownload_KeyUserActionWeb(t *testing.T) {
	...
	apiMap := api.APIs{
		api.KeyUserActionsWeb: standardAPIs[api.KeyUserActionsWeb],
		api.ApplicationWeb:    standardAPIs[api.ApplicationWeb],
	}
...
func TestDownload_SkippedParentsSkipChildren(t *testing.T) {
	...
	c := dtclient.NewMockClient(gomock.NewController(t))
	c.EXPECT().ListConfigs(gomock.Any(), apiEq(parentAPI)).Return([]dtclient.Value{{Id: "PARENT_ID_1", Name: "PARENT_NAME_1"}}, nil).Times(2)
