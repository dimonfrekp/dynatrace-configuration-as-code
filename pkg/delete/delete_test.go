...
func TestConfigsWithParent(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			mock: mockData{
				parentList: &listMock{
					api:      api.NewAPIs()[theAPI.Parent.ID],
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.mock.parentList != nil {
				client.EXPECT().ListConfigs(gomock.Any(), api.Eq(tc.mock.parentList.api)).Return(tc.mock.parentList.response, tc.mock.parentList.err).Times(1)
			}
			if tc.mock.list != nil {
				client.EXPECT().ListConfigs(gomock.Any(), api.Eq(tc.mock.list.api)).Return(tc.mock.list.response, tc.mock.list.err).Times(1)
			}
			if tc.mock.del != nil {
				client.EXPECT().DeleteConfigById(api.Eq(tc.mock.del.api), tc.mock.del.id).Return(tc.mock.del.err).Times(1)
			}
